...
public class ResultsPage extends BaseTest {
    ...
    public TrainInfo getTrainInfoByIndex(int index) {
        ...
        try {
            info.setCarriageNumber(driver.findElement(By.xpath(xpath.getCarriageNumber())).getText());//Получен номер вагона
        } catch (Exception e) {
            System.out.println("Ошибка получения номера вагона: " + e.getMessage());
        }

        try {
            info.setArrivalStation(driver.findElement(By.xpath(xpath.getArrivalStation())).getText());// Получена станция отправления и прибытия
        } catch (Exception e) {
            System.out.println("Ошибка получения станции отправления и прибытия: " + e.getMessage());
        }
        try {
            info.setDateArrival(driver.findElement(By.xpath(xpath.getArrivalDate())).getText());// Получена дата прибытия
        } catch (Exception e) {
            System.out.println("Ошибка получения даты прибытия: " + e.getMessage());
        }
        try {
            info.setArrivalTime(driver.findElement(By.xpath(xpath.getArrivalTime())).getText());// Получена время прибытия
        } catch (Exception e) {
            System.out.println("Ошибка получения времени прибытия: " + e.getMessage());
        }
        try {
            info.setPrice(driver.findElement(By.xpath(xpath.getPrice())).getText());// Получена цена
        } catch (Exception e) {
            System.out.println("Ошибка получения цены: " + e.getMessage());
        }
        try {
            info.setDateDeparture(driver.findElement(By.xpath(xpath.getDepartureDateDate())).getText());// Получена дата отправления
        } catch (Exception e) {
        ...
        try {
            info.setDepartureTime(driver.findElement(By.xpath(xpath.getDepartureDateTime())).getText());// Получено время отправления
        } catch (Exception e) {
            System.out.println("Ошибка получения времени отправления: " + e.getMessage());
        }
        try {
            info.setTravelTime(driver.findElement(By.xpath(xpath.getTravelTime())).getText());// Получено время в пути
        } catch (Exception e) {
            System.out.println("Ошибка получения времени в пути: " + e.getMessage());
        }
        try {
            info.setPlace(driver.findElement(By.xpath(xpath.getPlaceNumber())).getText());// Получен  номер места
        } catch (Exception e) {
            System.out.println("Ошибка получения номера места: " + e.getMessage());
        }
        try {
            info.setDepartureStation(driver.findElement(By.xpath(xpath.getDepartureStation())).getText());// Получено станция отправления
        } catch (Exception e) {
